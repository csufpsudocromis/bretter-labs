        ...
        <>
          <div className="card" style={{ marginBottom: '1rem' }}>
            ...
            <div className="specs">
              ...
              <span>Nodes Ready: {summary.ready_nodes ?? 0}/{summary.total_nodes ?? nodes.length}</span>
          ...
          <Bar label="CPU" used={cpuUsed} total={cpuTotal} formatter={fmtCpu} risk={risk.cpu} headroom={headroom.cpu_m || 0} />
          <Bar
            label="Memory"
            used={memUsed}
            total={memTotal}
              ...
              {recommendations.map((rec, idx) => {
                ...
                return (
                  <div key={`${rec.title}-${idx}`} className="tile template-tile">
                    ...
                    {rec.action && <div className="muted small" style={{ marginTop: '0.4rem' }}>Action: {rec.action}</div>}
                  </div>
                );
              })}
            </div>
          ...
          <div className="card">
            ...
            <div className="tile-grid" style={{ marginTop: '0.75rem' }}>
              {[
                { key: 'cpu', title: 'CPU', fmtReq: (item) => fmtCpu(item.requested?.cpu_m || 0), fmtUse: (item) => fmtCpu(item.usage?.cpu_m || 0) },
                {
                  key: 'memory',
                  title: 'Memory',
                ...
                <div key={section.key} className="tile template-tile">
                  ...
                  {(top[section.key] || []).slice(0, 5).map((item) => (
                    <div key={`${section.key}-${item.namespace}-${item.name}`} className="muted small" style={{ marginBottom: '0.35rem' }}>
                  ...
                  {!(top[section.key] || []).length && <div className="muted small">No requested resources reported.</div>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="tile-grid">
              ...
              {nodes.map((n) => (
                <div key={n.name} className="tile template-tile">
                  <div className="tile-header">
                    <h4>{n.name}</h4>
                    <span className={riskMeta(n.risk || 'healthy').className}>{(n.risk || 'healthy').toUpperCase()}</span>
                  </div>
                  <div className="specs">
                    ...
                    <span>CPU req: {fmtCpu((n.requested || n.usage || {}).cpu_m || 0)} / {fmtCpu((n.allocatable || {}).cpu_m || n.capacity_cpu_m || 0)}</span>
                    <span>RAM req: {fmtMem((n.requested || n.usage || {}).memory_bytes || (n.usage || {}).mem_bytes || 0)} / {fmtMem((n.allocatable || {}).memory_bytes || n.capacity_mem_bytes || 0)}</span>
                    <span>Disk req: {fmtMem((n.requested || n.usage || {}).disk_bytes || 0)} / {fmtMem((n.allocatable || {}).disk_bytes || n.capacity_disk_bytes || 0)}</span>
                    {n.usage && typeof n.usage.cpu_m === 'number' && <span>CPU use: {fmtCpu(n.usage.cpu_m)}</span>}
                    {n.usage && typeof n.usage.memory_bytes === 'number' && <span>RAM use: {fmtMem(n.usage.memory_bytes)}</span>}
                  </div>
            ...
            {!longhorn?.available ? (
              ...
              <>
                <div className="specs">
                  ...
                  <span>Used: {fmtMem(longhorn.used_bytes || 0)} / {fmtMem(longhorn.capacity_bytes || 0)}</span>
                ...
                {(longhorn.detached_volumes || 0) > 0 && (
                  <div className="muted small">
                    Note: detached volumes report robustness as unknown in Longhorn; this is expected when volumes are not attached.
                  </div>
                )}
              </>
            )}
