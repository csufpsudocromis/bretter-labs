...
USE_HELM_BASE_RELEASE="${USE_HELM_BASE_RELEASE:-auto}"
...
validate_helm_deploy_config() {
  ...
  case "$USE_HELM_BASE_RELEASE" in
    auto)
      if [ -d "$HELM_CHART_DIR" ] && [ -f "$HELM_CHART_DIR/Chart.yaml" ]; then
        USE_HELM_BASE_RELEASE=1
      else
        USE_HELM_BASE_RELEASE=0
      fi
      ;;
    0|1) ;;
    *)
      fail "USE_HELM_BASE_RELEASE must be auto, 0, or 1."
      ;;
  esac

  if [ "$USE_HELM_BASE_RELEASE" -eq 0 ]; then
    return
  fi
...
install_node() {
  ...
  if command -v node >/dev/null 2>&1; then
    if node_supports_vite8 "$(node -v)"; then
      need_node=0
    else
      log "Existing Node.js $(node -v) does not meet Vite 8 requirements; installing Node.js 20.x..."
    fi
  fi
...
install_metrics_server() {
  ...
  local patch_payload
  patch_payload="$(
    kubectl -n kube-system get deployment metrics-server -o json | python3 - "$METRICS_SERVER_INSECURE_TLS" <<'PY'
import json
import sys
...
render_manifest_template() {
  ...
  admin_bootstrap_password="$(escape_sed_replacement "$ADMIN_BOOTSTRAP_PASSWORD")"
  ...
  sed \
    ...
    -e "s#__ADMIN_BOOTSTRAP_PASSWORD__#${admin_bootstrap_password}#g" \
    "$input" >"$output"
}
...
apply_base_release_legacy_manifest() {
  log "Applying base release via legacy manifest: $RENDERED_APP_MANIFEST"
...
apply_manifests() {
  ...
  ensure_golden_images_claim
  reconcile_backend_data_pv
  if [ "$USE_HELM_BASE_RELEASE" -eq 1 ]; then
    reconcile_postgres_data_pv
    ensure_postgres_secret
  fi
  ensure_pull_secret
...
def request_json(method: str, path: str, **kwargs):
    ...
    return session.request(method=method, url=url, timeout=20, **kwargs)


def wait_for_instance(instance_id: str, allowed_states: set[str]) -> dict:
    while time.time() < DEADLINE:
        ...
        for row in rows:
            ...
            if state in allowed_states:
                return row
        time.sleep(3)
    fail(f"timeout while waiting for VM instance {instance_id}")
    return {}


def wait_until_vm_deleted(instance_id: str) -> None:
    while time.time() < DEADLINE:
        response = request_json("GET", "/user/pods")
        if response.status_code != 200:
            time.sleep(2)
            continue
        rows = response.json() or []
        if not any(str(row.get("id")) == instance_id for row in rows):
            return
        time.sleep(3)
    fail("VM instance was not deleted before timeout")
...
if not vm_templates:
    message = "synthetic check skipped: no enabled VM templates found"
...
vm_row = wait_for_instance(vm_id, {"pending", "running"})
...
wait_until_vm_deleted(vm_id)

print("Synthetic validation passed: login -> VM launch -> VM console URL -> VM delete.")
PY
EOF
...
main() {
  ...
  if [ "$USE_HELM_BASE_RELEASE" -eq 1 ]; then
    ...
    log "Helm base release disabled; using deploy/app.yaml manifest flow."
  fi
